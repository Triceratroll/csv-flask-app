...
def generate_customer_ranking(in_folder, out_folder):
    ...
    with open(os.path.join(in_folder, 'customers.csv'), 'r') as customers_file, open(os.path.join(in_folder, 'products.csv'), 'r') as products_file, open(os.path.join(in_folder, 'orders.csv'), 'r') as orders_file, open(os.path.join(out_folder, output_file), 'w', newline='') as customer_ranking_file:
